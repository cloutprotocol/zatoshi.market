...
vec3 dither(vec2 coord, vec3 color) {
  ...
  float step = 1.0 / (float(colorNum) - 1.0);
  color += threshold * step;
  float bias = 0.1;
  color = clamp(color - bias, 0.0, 1.0);
  return floor(color * (float(colorNum) - 1.0) + 0.5) / (float(colorNum) - 1.0);
}
...
void main() {
  ...
  // Apply dithering with 8x8 Bayer matrix
...
// Dithering and pixelation effects are implemented directly in the
// fragment shader using Bayer matrix dithering for retro visual style.
...
function DitheredWaves({
  ...
  return (
    <>
      ...
      {/* Transparent overlay mesh to capture pointer events */}
      <mesh
